...
## 8. Limitações Metodológicas e Ameaças à Validade

Para sustentação e defesa do trabalho perante a banca examinadora, registram-se de forma explícita as seguintes ameaças à validade interna e externa dos resultados:

1. **Ameaça à Validade Interna (Dependência Espacial do Split Original):** O benchmark oficial da dissertação ($\text{mAP@50} = 35.73\%$, $\text{Recall} = 68.40\%$) foi obtido sob a partição inicial do dataset, na qual os conjuntos de validação e teste não possuíam separação física independente (`val == test`) e não havia agrupamento por voos de drone (conforme auditado em [`docs/DATASET_SPLIT_AUDIT.md`](DATASET_SPLIT_AUDIT.md)). A ocorrência de imagens contíguas de um mesmo voo entre treino e validação pode introduzir correlação espacial e térmica, superestimando pontualmente o índice de generalização.
2. **Enquadramento como Piloto Operacional e Avaliação Preliminar (`experiments/revalidation_2026/`):** O presente estado de desenvolvimento do SolarGuard Vision constitui uma **fase de piloto operacional e prova de conceito quanto à maturidade das métricas de IA**, comprovando a acurácia dos módulos de calibração radiométrica de Planck e o pipeline de inferência. O dataset atual (52 imagens reais) serviu unicamente para a validação operacional do pipeline computacional, sendo os indicadores de $\text{mAP@50}$ e $\text{Recall}$ **preliminares e sujeitos a revisão substancial** quando a base expandida for incorporada. A revalidação preliminar em split independente por voo (`run_revalidation_2026.py`) empregou um modelo **distinto**, treinado do zero em CPU por apenas 10 épocas, alcançando $\text{mAP@50} = 5.32\%$. Por se tratar de um treinamento intencionalmente enxuto que **não convergiu**, esse indicador numérico não reflete o potencial real da rede sobre o novo particionamento, servindo unicamente para validar a operação do pipeline e dos cálculos de métricas com a classe `background`.
3. **Coleta do Dataset Definitivo em Andamento (Meta de 1000 a 1200 imagens):** Para a consolidação do sistema em escala industrial, encontra-se em andamento a **meta de ampliação e anotação padronizada de 1.000 a 1.200 imagens termográficas**, coletadas sob diversidade de irradiâncias solares, estações climáticas e diferentes topologias de usinas fotovoltaicas. Esse corpus expandido viabilizará o retreinamento do detector YOLOv11 com GPU por 100 épocas utilizando o particionamento baseado em voo, consolidando a avaliação final de generalização sobre o conjunto de teste cego (Flight A, 140 anotações).
4. **Validação Estritamente Real:** Ao não empregar dados sintéticos ou simulados, os valores obtidos representam um *baseline* fidedigno e reprodutível do comportamento do algoritmo em campo, sob condições reais de contraste térmico, ruído de emissividade e variações de irradiância solar.
